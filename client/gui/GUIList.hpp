...
public:
   GUIList(T &instance, void (T::*func)(std::string const &), ButtonSprite &left_arrow, ButtonSprite &middle_sprite, ButtonSprite &right_arrow, int x, int y)
    : GUIElement(x, y, 0, 0),  _leftArrow(&left_arrow), _middleSprite(&middle_sprite), _rightArrow(&right_arrow), _instance(&instance), _func(func)
  {
    this->_focusLabel = this->_labels.begin();
  }

  GUIList(T &instance, void (T::*func)(std::string const &), ButtonSprite &left_arrow, ButtonSprite &middle_sprite, ButtonSprite &right_arrow, GUILayout *layout)
    : GUIElement(0, 0, left_arrow.getWidth() + middle_sprite.getWidth() + right_arrow.getWidth(), middle_sprite.getHeight(), layout), _leftArrow(&left_arrow), _middleSprite(&middle_sprite), _rightArrow(&right_arrow), _instance(&instance), _func(func)
  {
    this->_focusLabel = this->_labels.begin();
  }
